...
export function useMapSearch(contextKey?: string) {
  ...
  useEffect(() => {
    ...
    if (!bbox) {
      console.log('📍 No bbox selected - please draw a rectangle on the map first')
      return
    }
    ...
    if (queryText) {
      ...
      console.log('🔍 Triggering map search for query:', queryText)

      // Trigger search
      mapRef.current?.search(queryText).catch((error) => {
        console.error('Map search failed:', error)
      })
    }
  }, [thread, mapRef, contextKey])
}
